...
impl LanguageServer for Backend {
    ...
    async fn code_action(
        ...
        if let Some(instruction_actions) = additional_features::get_instruction_code_actions(&node, &document.content) {
        ...
        match node.kind() {
            "identifier" => {
                ...
                if let Some(doc) = instructions::LOGIC_TYPE_DOCS.get(name) {
                    return Ok(Some(Hover {
                        contents: HoverContents::Array(vec![
                            MarkedString::String(format!("# `{}` (`logicType`)\n{}", name, doc)),
                        ]),
                        range: Some(Range::from(node.range()).into()),
                    }));
                }
                if let Some(doc) = instructions::SLOT_TYPE_DOCS.get(name) {
                    return Ok(Some(Hover {
                        contents: HoverContents::Array(vec![
                            MarkedString::String(format!("# `{}` (`logicSlotType`)\n{}", name, doc)),
                        ]),
                        range: Some(Range::from(node.range()).into()),
                    }));
                }
                if let Some(doc) = instructions::BATCH_MODE_DOCS.get(name) {
                    return Ok(Some(Hover {
                        contents: HoverContents::Array(vec![
                            MarkedString::String(format!("# `{}` (`batchMode`)\n{}", name, doc)),
                        ]),
                        range: Some(Range::from(node.range()).into()),
                    }));
                }
                    ...
                    if let AliasValue::Register(_) = &definition_data.value {
                        ...
                        if let Some(tree) = document.tree.as_ref() {
                            ...
                            if let Some(register_info) = register_analyzer.get_register_info(name) {
                                ...
                                value_parts.push(format!("**Register** {} ({})", name, register_name));
                                ...
                                if !register_info.operation_history.is_empty() {
                                    ...
                                    for record in &register_info.operation_history[start_idx..] {
                                        value_parts.push(format!("  • Line {}: {}", record.line_number, record.operation));
                                    }
                                    
                                    if start_idx > 0 {
                                        value_parts.push(format!("  • ... ({} earlier operations)", start_idx));
                                    }
                                } else {
                                    value_parts.push("**Operation history:** No operations found".to_string());
                                }
            ...
            "operation" | "invalid_instruction" => {
                if let Some(_signature) = instructions::INSTRUCTIONS.get(name) {
            ...
            if let Some(operation_node) = capture.child_by_field_name("operation") {
                ...
                for operand in operands {
                    ...
                    let typ = match operand_kind {
                        "register" => {
                            // Direct register
                            if let Some(reg_text) = operand.named_child(0).map(|n| n.utf8_text(document.content.as_bytes()).unwrap_or("") ) {
                        ...
                        "identifier" => {
                            let ident = operand
                                ...
                                .utf8_text(document.content.as_bytes())
                                .unwrap();
                            // Helper closure for case-insensitive membership (for user-typed variants)
                            let in_set_ci = |set: &phf::Set<&'static str>, value: &str| {
                                set.contains(value) || set.iter().any(|x| x.eq_ignore_ascii_case(value))
                            };
                            // If identifier text matches any known logic/slot/batch/reagent types (case-insensitive), treat as such
                            if in_set_ci(&instructions::LOGIC_TYPES, ident)
                                || in_set_ci(&instructions::SLOT_LOGIC_TYPES, ident)
                                || in_set_ci(&instructions::BATCH_MODES, ident)
                                || in_set_ci(&instructions::REAGENT_MODES, ident)
                            {
                                let lt = in_set_ci(&instructions::LOGIC_TYPES, ident);
                                let slot = in_set_ci(&instructions::SLOT_LOGIC_TYPES, ident);
                                let batch = in_set_ci(&instructions::BATCH_MODES, ident);
                                let reagent = in_set_ci(&instructions::REAGENT_MODES, ident);
                                if lt && slot {
                                    instructions::Union(&[DataType::LogicType, DataType::SlotLogicType])
                                } else if lt {
                                    instructions::Union(&[DataType::LogicType])
                                } else if slot {
                                    instructions::Union(&[DataType::SlotLogicType])
                                } else if batch {
                                    instructions::Union(&[DataType::BatchMode])
                                } else if reagent {
                                    instructions::Union(&[DataType::ReagentMode])
                                } else {
                                    unreachable!("Identifier membership logic inconsistent")
                                }
                            } else if parameter.match_type(DataType::Name) {
                    ...
                    if parameter.match_type(DataType::Device) {
                        if let Some(reg_name) = underlying_register.as_ref() {
                            if register_analyzer.get_register_kind(reg_name) == additional_features::ValueKind::DeviceId {
                    ...
                    if !parameter.match_union(&effective_typ) {
                        diagnostics.push(Diagnostic::new(
                            ...
                            None,
                            None,
                            format!("Type mismatch. Found {}, expected {}", effective_typ, parameter),
                            None,
                            None,
                        ));
                    }
